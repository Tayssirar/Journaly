...
import { theadExamData } from '../../data/TheadData'
...
function AllExams() {
  ...
  return (
    <div>
        <PageTitle activeMenu={"Tous les évaluations"} motherMenu={"Évaluation"}/>
        <Row>
                ...
                <Tab.Pane eventKey="List" className="col-lg-12">
                  <div className='card'>
                    <div className="card-header">
                      <h4 className="card-title">Toutes les évaluations </h4>
                      <Link to={"/AddExam"} className="btn btn-primary">+ Ajouter Un Nouveau</Link>
                    </div>
                              ...
                              <tbody>
                                {exams.map((data, ind)=>(
                                  <tr key={ind}>
                                    ...
                                    <td>{data.status}</td>
                                    <td>
                                      <Link to={"/UpdateExam"} className="btn btn-xs sharp btn-primary me-1"><i className="fa fa-pencil" /></Link>
                                      <Link to={"/DeleteExam"} className="btn btn-xs sharp btn-danger"><i className="fa fa-trash" /></Link>
                                    </td>
                                  </tr>
                                ))}
                              </tbody>
                        ...
                        <div className="card-header justify-content-end pb-0 border-0">
                          <Dropdown>
                            ...
                            <Dropdown.Menu align="end" className="dropdown-menu dropdown-menu-right border py-0">
                              <div className="py-2">
                                <Link to={"/UpdateExam"} className="dropdown-item">Modifier</Link>
                                <Link to={"/DeleteExam"} className="dropdown-item text-danger">Supprimer</Link>
                              </div>
                            </Dropdown.Menu>
                          </Dropdown>
                        </div>
                        <div className="card-body pt-2">
                          <div className="text-center">
                            <ul className="list-group mb-3 list-group-flush">
                              {Object.keys(Exam).map((key, ind) => (
                                <li className="list-group-item px-0 d-flex justify-content-between" key={ind}>
                                 <strong> {key}: </strong>{Exam[key]}
                                </li>
                              ))}
                            </ul>
                            <Link to={"/UpdateExam"} className="btn btn-outline-primary btn-rounded mt-3 px-4">Afficher</Link>
                          </div>
                        </div>
                      </div>
                    </div>
