...
import { theadHeadmasterData } from '../../components/UserTheadData'
...
const onClick = (i) => {
  ...
  function DataSearch(e){  
    const updatesData =  headmasters.filter(item =>{            
        let selectdata = `${item.nom} ${item.region} ${item.sexe}  ${item.mobile} ${item.email}`.toLowerCase();                          
        return  selectdata.includes(e.target.value.toLowerCase())
    });        
    setHeadmasters([...updatesData]);
    setActivePage(0);        
}
                      ...
                      <div  className='dataTables_wrapper no-footer'>
                        <div className='table-responsive '>
                          ...
                          <table  className='className="display dataTable no-footer w-100'>
                              <thead>
                                <tr>                                                
                                  {theadHeadmasterData.map((item, ind)=>(
                                    <th key={ind}
                                    >{item.heading}
                                    </th>
                                  ))}                                               
                              ...
                              <tbody>
                                {headmasters.map((data, ind)=>(
                                  <tr key={ind}>
                                    ...
                                    <td>{data.email}</td>
                                    <td>
                                      <Link to={"/UpdateHeadmaster"} className="btn btn-xs sharp btn-primary me-1"><i className="fa fa-pencil" /></Link>
                                      <Link to={"/DeleteHeadmaster"} className="btn btn-xs sharp btn-danger"><i className="fa fa-trash" /></Link>
                                    </td>
                                  </tr>
                                ))}
                              </tbody>
                        ...
                        <div className="card-header justify-content-end pb-0 border-0">
                          <Dropdown>
                            ...
                            <Dropdown.Menu align="end" className="dropdown-menu dropdown-menu-right border py-0">
                              <div className="py-2">
                                <Link to={"/UpdateHeadmaster"} className="dropdown-item">Modifier</Link>
                                <Link to={"/DeleteHeadmaster"} className="dropdown-item text-danger">Supprimer</Link>
                              </div>
                            </Dropdown.Menu>
                          </Dropdown>
                        </div>
                        <div className="card-body pt-2">
                          <div className="text-center">
                            <div className="profile-photo">
                              <img src={Headmaster.profile} width="100" className="img-fluid rounded-circle" alt="" />
                            </div>
                            <ul className="list-group mb-3 list-group-flush">
                              {Object.keys(Headmaster).map((key, ind) => (
                                <li className="list-group-item px-0 d-flex justify-content-between" key={ind}>
                                 <strong> {key}: </strong>{Headmaster[key]}
                                </li>
                              ))}
                            </ul>
                            <Link to={"/HeadmasterProfile"} className="btn btn-outline-primary btn-rounded mt-3 px-4">Afficher Profil</Link>
                          </div>
                        </div>
                      </div>
                    </div>
