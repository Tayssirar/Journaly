...
function AllResults() {
    ...
    function DataSearch(e){  
        const updatesData =  results.filter(item =>{            
            let selectdata = `${item.num} ${item.classe} ${item.groupe}  ${item.evaluation} ${item.creationDate}${item.updateDate} ${item.status}`.toLowerCase();                          
                              ...
                              <tbody>
                                {results.map((data, ind)=>(
                                  <tr key={ind}>
                                    ...
                                    <td>{data.status}</td>
                                    <td>
                                      <Link to={"/UpdateResults"} className="btn btn-xs sharp btn-primary me-1"><i className="fa fa-pencil" /></Link>
                                      <Link to={"/DeleteResults"} className="btn btn-xs sharp btn-danger"><i className="fa fa-trash" /></Link>
                                    </td>
                                  </tr>
                                ))}
                              </tbody>
                      ...
                      <div className="card card-profile">
                        <div className="card-header justify-content-end pb-0 border-0">
                          <Dropdown>
                            ...
                            <Dropdown.Menu align="end" className="dropdown-menu dropdown-menu-right border py-0">
                              <div className="py-2">
                                <Link to={"/UpdateResults"} className="dropdown-item">Modifier</Link>
                                <Link to={"/DeleteResults"} className="dropdown-item text-danger">Supprimer</Link>
                              </div>
                            </Dropdown.Menu>
                          </Dropdown>
                        </div>
                        <div className="card-body pt-2">
                          <div className="text-center">
                            ...
                            <Link to={"/UpdateResults"} className="btn btn-outline-primary btn-rounded mt-3 px-4">Afficher</Link>
                          </div>
                        </div>
                      </div>
                    </div>
