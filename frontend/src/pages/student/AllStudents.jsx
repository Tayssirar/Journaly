...
import { theadStudentData } from '../../components/UserTheadData'
...
const onClick = (i) => {
  ...
  function DataSearch(e){  
    const updatesData =  students.filter(item =>{            
        let selectdata = `${item.nom} ${item.region} ${item.sexe} ${item.classe} ${item.groupe} ${item.birthdate} `.toLowerCase();                          
        return  selectdata.includes(e.target.value.toLowerCase())
    });        
    setStudents([...updatesData]);
    setActivePage(0);        
}
  return (
    <div>
                ...
                <Tab.Pane eventKey="List" className="col-lg-12">
                  <div className='card'>
                    <div className="card-header">
                      <h4 className="card-title">Tous les élèves </h4>
                              ...
                              <tbody>
                                {students.map((data, ind)=>(
                                  <tr key={ind}>
                                    ...
                                    <td>
                                      <Link to={"/UpdateStudent"} className="btn btn-xs sharp btn-primary me-1"><i className="fa fa-pencil" /></Link>
                                      <Link to={"/DeleteStudent"} className="btn btn-xs sharp btn-danger"><i className="fa fa-trash" /></Link>
                                    </td>
                                  </tr>
                                ))}
                              </tbody>
                        ...
                        <div className="card-header justify-content-end pb-0 border-0">
                          <Dropdown>
                            ...
                            <Dropdown.Menu align="end" className="dropdown-menu dropdown-menu-right border py-0">
                              <div className="py-2">
                                <Link to={"/UpdateStudent"} className="dropdown-item">Modifier</Link>
                                <Link to={"/DeleteStudent"} className="dropdown-item text-danger">Supprimer</Link>
                              </div>
                            </Dropdown.Menu>
                          </Dropdown>
                        </div>
                        <div className="card-body pt-2">
                          <div className="text-center">
                            <div className="profile-photo">
                              <img src={student.profile} width="100" className="img-fluid rounded-circle" alt="" />
                            </div>
                            ...
                            <Link to={"/StudentProfile"} className="btn btn-outline-primary btn-rounded mt-3 px-4">Afficher Profil</Link>
                          </div>
                        </div>
                      </div>
                    </div>
