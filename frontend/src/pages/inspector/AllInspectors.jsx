...
import { theadInspectorData } from '../../components/UserTheadData'
...
const onClick = (i) => {
  ...
  function DataSearch(e){  
    const updatesData =  inspectors.filter(item =>{            
                              ...
                              <tbody>
                                {inspectors.map((data, ind)=>(
                                  <tr key={ind}>
                                    ...
                                    <td>{data.email}</td>
                                    <td>
                                      <Link to={"/UpdateInspector"} className="btn btn-xs sharp btn-primary me-1"><i className="fa fa-pencil" /></Link>
                                      <Link to={"/DeleteInspector"} className="btn btn-xs sharp btn-danger"><i className="fa fa-trash" /></Link>
                                    </td>
                                  </tr>
                                ))}
                              </tbody>
                        ...
                        <div className="card-header justify-content-end pb-0 border-0">
                          <Dropdown>
                            ...
                            <Dropdown.Menu align="end" className="dropdown-menu dropdown-menu-right border py-0">
                              <div className="py-2">
                                <Link to={"/UpdateInspector"} className="dropdown-item">Modifier</Link>
                                <Link to={"/DeleteInspector"} className="dropdown-item text-danger">Supprimer</Link>
                              </div>
                            </Dropdown.Menu>
                          </Dropdown>
                        </div>
                        <div className="card-body pt-2">
                          <div className="text-center">
                            ...
                            <ul className="list-group mb-3 list-group-flush">
                              {Object.keys(inspector).map((key, ind) => (
                                <li className="list-group-item px-0 d-flex justify-content-between" key={ind}>
                                 <strong> {key}: </strong>{inspector[key]}
                                </li>
                              ))}
                            </ul>
                            <Link to={"/InspectorProfile"} className="btn btn-outline-primary btn-rounded mt-3 px-4">Afficher Profil</Link>
                          </div>
                        </div>
                      </div>
                    </div>
