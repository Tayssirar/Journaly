...
import { theadAssistantData } from '../../components/UserTheadData'
...
const onClick = (i) => {
  ...
  function DataSearch(e){  
    const updatesData =  assistants.filter(item =>{            
                              ...
                              <tbody>
                                {assistants.map((data, ind)=>(
                                  <tr key={ind}>
                                    ...
                                    <td>{data.email}</td>
                                    <td>
                                      <Link to={"/UpdateAssistant"} className="btn btn-xs sharp btn-primary me-1"><i className="fa fa-pencil" /></Link>
                                      <Link to={"/DeleteAssistant"} className="btn btn-xs sharp btn-danger"><i className="fa fa-trash" /></Link>
                                    </td>
                                  </tr>
                                ))}
                              </tbody>
                        ...
                        <div className="card-header justify-content-end pb-0 border-0">
                          <Dropdown>
                            ...
                            <Dropdown.Menu align="end" className="dropdown-menu dropdown-menu-right border py-0">
                              <div className="py-2">
                                <Link to={"/UpdateAssistant"} className="dropdown-item">Modifier</Link>
                                <Link to={"/DeleteAssistant"} className="dropdown-item text-danger">Supprimer</Link>
                              </div>
                            </Dropdown.Menu>
                          </Dropdown>
                        </div>
                        <div className="card-body pt-2">
                          <div className="text-center">
                            <div className="profile-photo">
                              <img src={assistant.profile} width="100" className="img-fluid rounded-circle" alt="" />
                            </div>
                            <ul className="list-group mb-3 list-group-flush">
                              {Object.keys(assistant).map((key, ind) => (
                                <li className="list-group-item px-0 d-flex justify-content-between" key={ind}>
                                 <strong> {key}: </strong>{assistant[key]}
                                </li>
                              ))}
                            </ul>
                            <Link to={"/AssistantProfile"} className="btn btn-outline-primary btn-rounded mt-3 px-4">Afficher Profil</Link>
                          </div>
                        </div>
                      </div>
                    </div>
